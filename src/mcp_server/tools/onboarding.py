...
from mcp_server.identity import get_service_manager, ensure_installed, _LOCAL_MODE
...
async def get_github_install_url(ctx: Context) -> dict:
    ...
    state = base64.urlsafe_b64encode(sm.user_id.encode()).decode()
    url = f"https://github.com/apps/{github_app_name}/installations/new?state={state}"
    return {"install_url": url, "user_id": sm.user_id}
...
async def index_repository(ctx: Context, repository_name: str) -> dict:
    ...
    """
    sm = await get_service_manager(ctx)
    ensure_installed(sm.user_id)

    if not _LOCAL_MODE:
        repo_db = UserRepositoryDB()
        if not repo_db.find_user_repository(sm.user_id, repository_name):
...
async def list_user_repositories(ctx: Context, indexed_only: bool = False) -> list:
    ...
    repos = UserRepositoryDB().find_user_repositories(sm.user_id)
    if indexed_only:
